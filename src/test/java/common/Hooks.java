...
import org.testng.asserts.SoftAssert; // Keeping TestNG's SoftAssert as per your import
...
@SuppressWarnings("all") // Consider removing or narrowing this after fixing all specific warnings
public class Hooks {
    ...
    public static SoftAssert softAssertions = new SoftAssert(); // Initializing TestNG's SoftAssert
    ...
    private static void clearDirectoryContents(File directory) {
        ...
        if (files != null) {
            for (File file : files) {
                ...
                // Use FileUtils.deleteQuietly for robustness or ensure proper exception handling
                file.delete();
            }
        }
    }
    ...
    public void init(Scenario scenario) {
        ...
        if (apiurl != null && !apiurl.isEmpty()) {
            RestAssuredUtil.setApiCmdUrl(apiurl);
        }
    ...
    public void storeScenario(Scenario scenario) {
        ...
        WebBrowserUtil.sce = scenario; // Potential issue with parallel execution, consider ThreadLocal

        try {
            // Access private field 'delegate' - this is a fragile approach
            ...
            // Access private field 'testCase' - this is a fragile approach
            ...
            // Filter out before/after hooks to get actual step definitions
            stepDefs = testCase.getTestSteps().stream()
            ...
            log.error("Error accessing scenario details (reflection issue): ", e);
        }
    ...
    public void addScreenshot(Scenario scenario) {
        if (consoleOutput) {
            System.out.println("the step name: " + StepListener.getStepName());
        }
        ...
        if (storeScreenshot && StepListener.getStepName().toLowerCase().contains("verify")) {
            ...
            String sanitizedStepName = StepListener.getStepName().replaceAll("[^a-zA-Z0-9_-]", "_");
    ...
    public void afterScenario(Scenario scenario) {
        Assertion.assertAll(); // This will call softAssertions.assertAll()
        ...
        softAssertions = new SoftAssert(); // Resetting soft assertions for the next scenario

        boolean semiAuto = false; // This variable is passed to processScenarioTags, but its modification here won't affect the original 'semiAuto' in this method scope. If it's meant to be global, make it static.
        ...
        processScenarioTags(scenario, semiAuto); // 'semiAuto' might be misinterpreted here if it was meant to be stateful.

        scenarioName = true;
        softAssertions = new SoftAssert(); // Resetting soft assertions again, redundant with above.
    }
        ...
        try {
            ...
            for (String tag : uniqueTag) {
                if (tag.contains("@test")) {
                    if (test.length() > 0) // CORRECTED: Use length() for StringBuilder
                        test.append(",");
                    test.append(tag);
                } else if (tag.contains("@set")) {
                    if (set.length() > 0) // CORRECTED: Use length() for StringBuilder
                        set.append(",");
                    set.append(tag);
                } else {
                    if (otherTags.length() > 0) // CORRECTED: Use length() for StringBuilder
                        otherTags.append(",");
    ...
    private void handleVideoRecording(Scenario scenario) {
        ...
        // Determine whether to delete the recording based on success/failure capture settings
    ...
    private void processScenarioTags(Scenario scenario, boolean semiAuto) {
        // The 'semiAuto' parameter here seems to be local and won't affect the 'semiAuto' in the calling 'afterScenario' method.
        // If it's intended to be a global state, it should be a static field.
        for (String tag : scenario.getSourceTagNames()) {
            if (shouldCloseInstance(tag)) {
                WebBrowser.closeBrowserInstance();
            }
            if (tag.contains("semiauto")) {
                semiAuto = true; // This only sets the local 'semiAuto' within this method scope.
            }
    ...
    private void updateTFSResults(String tag) {
        if (tfsResult && tag.contains("test")) {
            int testCaseId = 0; // This testCaseId needs to be dynamically extracted from the scenario or tag. Hardcoding 0 is likely incorrect.
